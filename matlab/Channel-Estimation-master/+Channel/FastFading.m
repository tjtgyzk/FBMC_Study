classdef FastFading < handle
    % =====================================================================        
    % 此MATLAB类表示时变通道的实现。通道参数由类构造函数初始化。 卷积通过方法
    % ".Convolution(.)". 通过 ".NewRealization;"获得新的信道实现. 此外，还有
    % 一些其他有用的方法, such as ".GetRmsDelaySpread" or
    % ".PlotPowerDelayProfile"
    % =====================================================================    
    ...
    methods
        function obj = FastFading(...
                SamplingRate,...                                   % 抽样率 (Samples/s)
                PowerDelayProfile,...                              % 功率延迟配置文件，字符串或向量： 'Flat', 'AWGN', 'PedestrianA', 'PedestrianB', 'VehicularA', 'VehicularB', 'ExtendedPedestrianA', 'ExtendedPedestrianB', or 'TDL-A_xxns','TDL-B_xxns','TDL-C_xxns' (with xx the RMS delay spread in ns, e.g. 'TDL-A_30ns'), or [1 0 0.2] (Self-defined power delay profile which depends on the sampling rate)
                SamplesTotal,...                                   % 总样本数
                MaximumDopplerShift,...                            % 最大多普勒频移: Velocity_kmh/3.6*CarrierFrequency/2.998e8
                DopplerModel,...                                   % 多普勒模型: 'Jakes', 'Uniform', 'Discrete-Jakes', 'Discrete-Uniform'. For "Discrete-", we assume a discrete Doppler spectrum to improve the simulation time. This only works accuratly if the number of samples and the velocity is sufficiently large
                Paths,...                                          % WSSUS进程的路径数.仅与“Jakes”和“Uniform”多普勒频谱相关
                nTxAntennas,...                                    % 发射天线的数量
                nRxAntennas,...                                    % 接收天线的数量
                WarningIfSampleRateDoesNotMatch ...                % 如果通道的预定义延迟抽头不符合采样率，则发出警告。如果它们大致相同，这通常不是什么大问题。
                )
        ...
        function NewRealization( obj )
            ...
            if strcmp(obj.Implementation.PowerDelayProfile, 'AWGN')
                ...
                if obj.PHY.MaximumDopplerShift>0
                    ...
                    if obj.Implementation.UseDiscreteDopplerSpectrum % 使用IFFT进行更快的计算。基本假设：离散多普勒频移=>仅对大量样本/多普勒频移有效
        ...
        function convolvedSignal = Convolution(obj, signal)
            ...
            if obj.PHY.MaximumDopplerShift>0
                ...
                convolvedSignal = convolvedSignal(1:N,:);% 去掉尾巴
            end
        end
        ...
        function ChannelTransferFunction = GetTransferFunction(obj, TimePos, FFTSize, ActiveSubcarrier)
            % 计算特定时间位置的通道传递函数。第一个参数是向量，表示时间位置。
            % 第二个参数表示基础调制格式的FFT大小。第三个参数（如果指定）确定哪
            % 些子载波是活动的（只返回这些频率位置）。
