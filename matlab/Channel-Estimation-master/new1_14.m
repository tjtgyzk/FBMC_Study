...
NrRepetitions             = 1000;                     % 蒙特卡罗重复次数（不同信道实现）             
...
for i_rep = 1:NrRepetitions
    ...
    BinaryDataStream_FBMC_Aux_Coded = convenc(BinaryDataStream_FBMC_Aux,trel);
    %% 数据符号
    xD_FBMC_Aux = QAM.Bit2Symbol(BinaryDataStream_FBMC_Aux_Coded);     
    %% 导频符号
    ...
    Chennel = comm.RayleighChannel('SampleRate',SamplingRate, ...
        ...
        DetectedBitStream_OneTapEqualizer_FBMC_Aux = QAM.Symbol2Bit(xD_est_OneTapEqualizer_FBMC_Aux);
      ...
      %  BER_FBMC_Aux_OneTapEqualizer(i_SNR,i_rep) = mean(BinaryDataStream_FBMC_Aux~=DetectedBitStream_OneTapEqualizer_FBMC_Aux_De); 
        BER_FBMC_Aux_OneTapEqualizer_C(i_SNR,i_rep) = mean(BinaryDataStream_FBMC_Aux(1:end - tblen)~=DetectedBitStream_OneTapEqualizer_FBMC_Aux_De(tblen+1:end)); 
        %% 迭代
    ...
    xP1_FBMC = xP1_FBMC./abs(xP1_FBMC);
        ...
        x1_FBMC_Aux = AuxiliaryMethod.PrecodingMatrix*[xP1_FBMC;xD1_FBMC_Aux];
        ...
        hP1_est_FBMC_Aux = y1_FBMC_Aux(PilotMatrix_FBMC==1)./xP1_FBMC/sqrt(Kappa_Aux);
     ...
     %% 数据恢复
        x1_est_OneTapEqualizer_FBMC_Aux = y1_FBMC_Aux./h1_est_interp;
    ...
    figure(1);
    semilogy(M_SNR_dB, nanmean(BER_FBMC_Aux_OneTapEqualizer_C(:,:),2),'red',M_SNR_dB, nanmean(BER1_FBMC_Aux_OneTapEqualizer_C(:,:),2),'blue');
