...
for i_rep = 1:NrRepetitions
  ...
  % channel = comm.RayleighChannel("SampleRate",SamplingRate);
   r_FBMC_Aux_noNoise = ConvolutionMatrix*s_FBMC_Aux;
 %  r_FBMC_Aux_noNoise = channel(s_FBMC_Aux);
  % h = r_FBMC_Aux_noNoise./s_FBMC_Aux;
    ...
    for i_SNR = 1:length(M_SNR_dB)
        ...
        %hP_est_FBMC_Aux = real(y_FBMC_Aux(PilotMatrix_FBMC==1)./xP_FBMC/sqrt(Kappa_Aux));
        %% 估计传输矩阵
       ...
       % h_est_interp = h_est_interp';
        ...
        %D_FBMC_est_Aux = diag(h_est_interp)*Q_FBMC'*G_FBMC;
        %% 单抽头均衡器
    ...
    %    xD_est_FBMC_Aux_PerfectCSI_Temp = xD_est_OneTapEqualizer_FBMC_Aux_PerfectCSI; % 单抽头估计值初始化     
        ...
        h_est_FBMC_Aux_Temp  = h_est_FBMC_Aux;
         for i_iteration = 1:NrIterations
            ...
            %h_est_interp_Temp = interp1(index,hP_est_FBMC_Aux_Temp',(1:720),'pchip');
            %h_est_interp_Temp = h_est_interp';
            %D_FBMC_est_Aux_Temp = diag(h_est_interp_Temp)*Q_FBMC'*G_FBMC;
            % 改进的信道估计（迭代）
